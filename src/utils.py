...
def calculate_results(refs, preds):
    ...
    for filterd_pred, filterd_ref in zip(preds, refs):

        if filterd_pred == filterd_ref:
            true_positive += len(filterd_pred)
            continue

        for fp in filterd_pred.split():
            if fp in filterd_ref.split():
                true_positive += 1
                
            else:
                false_positive += 1

        for fr in filterd_ref.split():
            if not fr in filterd_pred.split():
                false_negative += 1
...
def compute_bleu(ref_file_name, predicted_file_name):
    with open(predicted_file_name) as predicted_file:
        pipe = subprocess.Popen(["perl", "src/multi-bleu.perl", ref_file_name], stdin=predicted_file,                                                        stdout=sys.stdout, stderr=sys.stderr)